...
class FeeCalculator:
    ...
    def __init__(self, late_fee_per_day: float = 0.50, 
                 max_late_fee: float = 50.0):
        """
    ...
    def calculate_late_fee(self, due_date: datetime, 
                          return_date: Optional[datetime] = None) -> float:
        """
    ...
    def calculate_damage_fee(self, item_value: float, 
                            damage_percentage: float = 50.0) -> float:
        """
...
class FeeManager:
    ...
    def create_fee(self, member_id: str, fee_type: FeeType, 
                   amount: float, description: str,
                   due_date_offset_days: int = 30) -> Fee:
        """
    ...
    def get_member_fees(self, member_id: str) -> List[Fee]:
        ...
        """
        return [fee for fee in self.fees.values() 
                if fee.member_id == member_id]
    
    def get_unpaid_fees(self, member_id: str) -> List[Fee]:
        """Get unpaid fees for a member."""
        return [fee for fee in self.get_member_fees(member_id) 
                if not fee.is_paid]
    ...
    def get_overdue_fees(self) -> List[Fee]:
        ...
        now = datetime.now()
        return [fee for fee in self.fees.values() 
                if not fee.is_paid and fee.due_date < now]